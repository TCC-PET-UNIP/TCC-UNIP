...
class AuthService {
  ...
  async login(credentials: LoginRequest): Promise<LoginResponse> {
    try {
      ...
      const mockUser: UserProfile = {
        ...
        nome_fantasia: credentials.email.includes('ong') ? 'ONG Amor Animal' : undefined,
        cnpj: credentials.email.includes('ong') ? '12.345.678/0001-90' : undefined,
  ...
  // Validações comuns
  private validateRegistration(data: any): { isValid: boolean; message: string } {
    if (!data.email || !data.senha || !data.confirmarSenha) {
      return { isValid: false, message: 'Todos os campos obrigatórios devem ser preenchidos' };
    }
    ...
    if (!endereco.logradouro || !endereco.numero || !endereco.bairro || 
      ...
      return { isValid: false, message: 'Todos os campos do endereço são obrigatórios' };
    }
